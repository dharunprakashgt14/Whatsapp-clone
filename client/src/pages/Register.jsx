...
const Register = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const { data } = await sendOtp({ email, mode: 'register', name, channel: 'email' });
      setOtpSent(true);
      setDemoOtp(data.otp || '');
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await verifyOtp({ email, otp, mode: 'register', name, channel: 'email' });
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        ...
        <p className="auth-subtitle">Create your account with email OTP</p>
        ...
        <form className="auth-form" onSubmit={otpSent ? handleVerifyOtp : handleSendOtp}>
          ...
          {otpSent && (
            <button className="auth-btn" type="button" style={{ marginTop: 0 }} onClick={handleSendOtp} disabled={loading}>
              Resend OTP
            </button>
          )}
        </form>
